...
import { PropertyDict } from "../types.ts";

const reFloat = /[-+]?[0-9]*\.?[0-9]+/;

function atomNameListMap(x: string) {
  return x.split("+").map(B.atomName);
}
function listMap(x: string) {
  return x.split("+").map((x) => x.replace(/^["']|["']$/g, ""));
}
...
function elementListMap(x: string) {
  return x.split("+").map((symbol) => B.es(symbol));
}
...
export const properties: PropertyDict = {
  ...
  resn: {
    "@desc":
      "residue-name-list: list of 3-letter codes for amino acids or list of up to 2-letter codes for nucleic acids",
  ...
  ss: {
    "@desc":
      "secondary-structure-type list of single letters. Helical regions should be assigned H and sheet regions S. Loop regions can either be assigned L or be blank.",
    "@examples": ["ss H+S+L", 'ss S+""'],
    abbr: [],
    regex: /[a-zA-Z'"+]+/,
    map: sstrucListMap,
    level: "residue-test",
    property: B.ammp("secondaryStructureFlags"),
  },
  ...
  b: {
    ...
    map: (x) => parseFloat(x),
    level: "atom-test",
    property: B.ammp("B_iso_or_equiv"),
  },
  q: {
  ...
  formal_charge: {
    ...
    map: (x) => parseFloat(x),
    level: "atom-test",
    property: B.ammp("pdbx_formal_charge"),
  },
  partial_charge: {
