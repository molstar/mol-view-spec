 ...
 **
 * Vendored from Mol* src/mol-util/monadic-parser.ts at commit 33c74c721.
...
function mergeReplies<A, B>(result: MonadicParser.Result<A>, last?: MonadicParser.Result<B>): MonadicParser.Result<A> {
  ...
  const expected = result.furthest === last.furthest ? unsafeUnion(result.expected, last.expected) : last.expected;
