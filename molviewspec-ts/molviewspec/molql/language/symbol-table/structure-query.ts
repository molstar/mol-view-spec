...
export namespace Types {
  ...
  export const BondFlag = Type.OneOf("Structure", "BondFlag", Type.Str, [
    "covalent",
    "metallic",
    "ion",
    "hydrogen",
  ...
  export const SecondaryStructureFlag = Type.OneOf("Structure", "SecondaryStructureFlag", Type.Str, [
    "alpha",
    "beta",
    "3-10",
    "pi",
  ...
  export const EntityType = Type.OneOf("Structure", "EntityType", Type.Str, [
    "polymer",
    "non-polymer",
    "water",
    "branched",
  ]);
  export const EntitySubtype = Type.OneOf("Structure", "EntitySubtype", Type.Str, [
    "other",
    "polypeptide(D)",
    "polypeptide(L)",
  ...
  export const ObjectPrimitive = Type.OneOf("Structure", "ObjectPrimitive", Type.Str, [
    "atomistic",
    "sphere",
    "gaussian",
    "other",
...
const type = {
  ...
  atomName: symbol(
    Arguments.Dictionary({ 0: Argument(Type.AnyValue) }),
    Types.AtomName,
  ...
  bondFlags: symbol(
    ...
    Types.BondFlags,
    `Create bond flags representation from a list of strings. Allowed flags: ${
      Type.oneOfValues(Types.BondFlag).join(", ")
    }.`,
  ),
  ...
  secondaryStructureFlags: symbol(
    ...
    `Create secondary structure flags representation from a list of strings. Allowed flags: ${
      Type.oneOfValues(Types.SecondaryStructureFlag).join(", ")
    }.`,
  ),

  authResidueId: symbol(
...
const slot = {
  ...
  elementSetReduce: symbol(
    ...
    "Current value of the element set reducer.",
  ),
};
...
const generator = {
  ...
  atomGroups: symbol(
    ...
    Types.ElementSelectionQuery,
    "Return all atoms for which the tests are satisfied, grouped into sets.",
  ),

  bondedAtomicPairs: symbol(
    Arguments.Dictionary({
      0: Argument(Type.Bool, {
        isOptional: true,
        defaultValue: "true for covalent bonds" as any,
    ...
    Types.ElementSelectionQuery,
    "Return all pairs of atoms for which the test is satisfied.",
  ),
  ...
  rings: symbol(
    ...
    Types.ElementSelectionQuery,
    "Return all rings or those with the specified fingerprint and/or only aromatic rings.",
  ),

  queryInSelection: symbol(
    Arguments.Dictionary({
...
const modifier = {
  ...
  exceptBy: symbol(
    ...
    Types.ElementSelectionQuery,
    `Remove all atoms from 'selection.ts' that occur in 'by'.`,
  ),
  ...
  includeSurroundings: symbol(
    Arguments.Dictionary({
      ...
      "atom-radius": Argument(Type.Num, {
        ...
        defaultValue: 0,
        description:
          "Value added to each atom before the distance check, for example VDW radius. Using this argument is computationally demanding.",
      }),
  ...
  surroundingLigands: symbol(
    ...
    Types.ElementSelectionQuery,
    "Find all ligands components around the source query.",
  ),

  includeConnected: symbol(
    Arguments.Dictionary({
      0: Argument(Types.ElementSelectionQuery),
  ...
  wholeResidues: symbol(
    Arguments.Dictionary({
      0: Argument(Types.ElementSelectionQuery),
    }),
    Types.ElementSelectionQuery,
    "Expand the selection to whole residues.",
  ),

  expandProperty: symbol(
    Arguments.Dictionary({
      0: Argument(Types.ElementSelectionQuery),
...
const filter = {
  ...
  first: symbol(
    ...
    Types.ElementSelectionQuery,
    "Take the 1st atom set in the sequence.",
  ),

  withSameAtomProperties: symbol(
    Arguments.Dictionary({
      0: Argument(Types.ElementSelectionQuery),
...
const combinator = {
  ...
  distanceCluster: symbol(
    Arguments.Dictionary({
      matrix: Argument(Core.Types.List(Core.Types.List(Type.Num)), {
        description:
          "Distance matrix, represented as list of rows (num[][])). Lower triangle is min distance, upper triangle is max distance.",
      }),
...
const atomSet = {
  ...
  reduce: symbol(
    Arguments.Dictionary({
      initial: Argument(Type.Variable("a", Type.AnyValue, true), {
        description:
          "Initial value assigned to slot.atom-set-reduce. Current atom is set to the 1st atom of the current set for this.",
      }),
  ...
  propertySet: symbol(
    ...
    Core.Types.Set(Core.Types.ConstrainedVar),
...
const atomProperty = {
  ...
  macromolecular: {
    ...
    entityType: atomProp(
      Types.EntityType,
      "Type of the entity as defined in mmCIF (polymer, non-polymer, branched, water)",
    ),
    entitySubtype: atomProp(
      Types.EntitySubtype,
    ...
    objectPrimitive: atomProp(
      Types.ObjectPrimitive,
      "Type of the primitive object used to model this segment as defined in mmCIF/IHM (atomistic, sphere, gaussian, other)",
    ),
  ...
  ihm: {
    hasSeqId: symbol(
      ...
      Type.Bool,
      "Checks if the current element represents a given sequence id",
    ),
    overlapsSeqIdRange: symbol(
      Arguments.Dictionary({ beg: Argument(Type.Num), end: Argument(Type.Num) }),
      Type.Bool,
      "Checks if the current element overlaps with the specified residue range",
    ),
  },
};
